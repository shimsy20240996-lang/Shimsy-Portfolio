    ...
    <section id="home" className="relative min-h-screen flex items-center pt-24 pb-12 overflow-hidden bg-transparent">
      ...
      <div className="container mx-auto px-6 relative z-10 w-full">
        ...
        <motion.div 
          ...
          className="w-full max-w-5xl mx-auto h-[400px] sm:h-[450px] relative rounded-2xl overflow-hidden shadow-[0_0_40px_rgba(168,85,247,0.15)] mb-12 border border-border/10"
        >
          {/* Mac Terminal Layout */}
          <div className="absolute inset-0 bg-panel/90 backdrop-blur-xl flex flex-col">
            {/* Terminal Header */}
            <div className="h-10 bg-surface border-b border-border/10 flex items-center px-4 gap-2">
              <div className="w-3 h-3 rounded-full bg-red-500"></div>
              <div className="w-3 h-3 rounded-full bg-yellow-500"></div>
              <div className="w-3 h-3 rounded-full bg-green-500"></div>
              <div className="mx-auto text-xs text-text-muted font-mono flex-1 text-center pr-16">
                developer@portfolio:~
              </div>
            </div>
            ...
            <div className="flex-1 p-6 sm:p-8 font-mono text-sm sm:text-base md:text-lg overflow-hidden relative text-left">
              ...
              <div className="mt-4 min-h-[160px] whitespace-pre-wrap">
                <span className="text-cyan-400">const</span> <span className="text-text-main">developer</span> <span className="text-cyan-400">=</span> <span className="text-text-main">{'{'}</span>
                <br />
                ...
                {displayedCode.split('\n').length > 1 && (
                  <><span className="text-text-main">{displayedCode.split('\n').pop()}</span></>
                )}
                {!displayedCode.includes('\n') && (
                  <span className="text-text-main">{displayedCode.substring(17)}</span>
                )}
        ...
        <div className="text-center max-w-4xl mx-auto">
          ...
          <motion.h2 
            ...
            className="text-xl sm:text-2xl md:text-3xl font-medium text-text-muted mb-8 font-display"
          >
          ...
          >
            <Link to="projects" smooth={true} duration={500} offset={-80}>
              <button className="group flex items-center gap-2 bg-accent hover:bg-accent-light text-text-main px-8 py-3.5 rounded-full font-medium transition-all shadow-[0_0_20px_rgba(168,85,247,0.3)] hover:shadow-[0_0_30px_rgba(168,85,247,0.6)]">
                View Projects 
                <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
              </button>
            </Link>
            
            <a href="/cv.pdf" download className="flex items-center gap-2 px-8 py-3.5 rounded-full font-medium text-text-main border border-accent/30 bg-accent/10 hover:bg-accent/20 transition-all backdrop-blur-sm">
