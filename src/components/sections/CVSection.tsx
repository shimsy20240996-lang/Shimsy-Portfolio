    ...
    <section id="cv" className="py-24 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 md:px-12">
        ...
        >
          ...
          <a href="/cv.pdf" download className="group relative inline-flex items-center justify-center px-8 py-3.5 text-base font-bold text-slate-900 dark:text-white transition-all duration-200 bg-surface border border-slate-300 dark:border-white/10 rounded-xl hover:bg-slate-100 dark:hover:bg-white/5 hover:border-accent/50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent overflow-hidden">
            <span className="absolute inset-0 w-full h-full -mt-1 rounded-lg opacity-30 bg-gradient-to-b from-transparent via-transparent to-black dark:to-black/50"></span>
        ...
        <div className="glass-panel rounded-2xl border border-slate-200 dark:border-white/10 overflow-hidden flex flex-col lg:flex-row shadow-2xl">
          {/* Sidebar / Tabs */}
          <div className="lg:w-64 bg-surface/50 border-r border-slate-200 dark:border-white/10 p-6 flex flex-row lg:flex-col gap-2 overflow-x-auto">
          ...
          <div className="flex-1 p-8 lg:p-12 min-h-[400px]">
            <AnimatePresence mode="wait">
              {activeTab === 'overview' && (
                ...
                >
                  <h3 className="text-2xl font-bold mb-6 text-slate-900 dark:text-white border-b border-slate-200 dark:border-white/10 pb-4">Professional Summary</h3>
                  <div className="prose prose-slate dark:prose-invert max-w-none text-slate-700 dark:text-slate-300">
                    <p className="text-lg leading-relaxed mb-6">{portfolioData.personal.bio}</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                      <div className="bg-slate-100 dark:bg-surface/50 p-6 rounded-xl border border-slate-200 dark:border-white/5">
                        <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Role</p>
                        <p className="font-semibold text-lg text-slate-900 dark:text-white">{portfolioData.personal.role}</p>
                      </div>
                      <div className="bg-slate-100 dark:bg-surface/50 p-6 rounded-xl border border-slate-200 dark:border-white/5">
                        <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Degree</p>
                        <p className="font-semibold text-lg text-slate-900 dark:text-white">{portfolioData.education[0].degree}</p>
                      </div>
                    </div>
                  </div>
                </motion.div>
                  ...
                  <div className="space-y-8">
                    {portfolioData.education.map((edu) => (
                      <div key={edu.id} className="relative pl-8 before:absolute before:left-0 before:top-2 before:w-3 before:h-3 before:bg-accent before:rounded-full before:shadow-[0_0_10px_rgba(59,130,246,0.5)] after:absolute after:left-[5px] after:top-5 after:w-0.5 after:h-full after:bg-slate-300 dark:after:bg-white/10 last:after:hidden">
                        ...
                        {edu.description && <p className="text-slate-700 dark:text-slate-300">{edu.description}</p>}
                      </div>
                    ))}
                  </div>
                </motion.div>
              ...
              {activeTab === 'skills' && (
                <motion.div
                  ...
                  transition={{ duration: 0.3 }}
                >
                  <h3 className="text-2xl font-bold mb-6 text-slate-900 dark:text-white border-b border-slate-200 dark:border-white/10 pb-4">Skills Overview</h3>
                      ...
                      <div key={category.title}>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {category.skills.map(skill => (
                            <span key={skill.name} className="px-3 py-1.5 bg-surface rounded-lg border border-slate-300 dark:border-white/5 text-sm font-medium text-slate-700 dark:text-slate-300 hover:border-accent/50 transition-colors cursor-default">
                              {skill.name}
                            </span>
                          ))}
                        </div>
